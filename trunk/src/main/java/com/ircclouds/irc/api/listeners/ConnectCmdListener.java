...
public class ConnectCmdListener
{
	...
	{
		if (aMessage instanceof ServerMessage)
		{
			ServerMessage _msg = (ServerMessage) aMessage;
			if (_msg.getNumericCode() == IRCServerNumerics.NICKNAME_IN_USE)
			{
			...
			else if (_msg.getNumericCode() == IRCServerNumerics.ERRONEUS_NICKNAME)
			{
				throw new RuntimeException("Errorneus nickname");
			}
			else if (_msg.getNumericCode() == IRCServerNumerics.SERVER_WELCOME_MESSAGE)
			{
				nick = getNick(_msg.getText());
			}
			else if (_msg.getNumericCode().equals(IRCServerNumerics.SERVER_OPTIONS))
			{
				String _opts[] = ((ServerMessage) aMessage).getText().split(" ");
			...
			else if (_msg.getNumericCode().equals(IRCServerNumerics.MOTD_FILE_MISSING) || _msg.getNumericCode().equals(IRCServerNumerics.END_OF_MOTD))
			{
				callback.onSuccess(new IRCStateImpl(nick, params.getIdent(), params.getRealname(), params.getAlternativeNicknames(),
						params.getServer(), new IRCServerOptions(properties)));
			}
		}
		else if (aMessage instanceof ErrorMessage)
		{
			callback.onFailure(((ErrorMessage) aMessage).getText());
		}
	}
